...
function getCookie(name){
  ...
  // Fallback demo (no BE)
  async function fakeFetch(){ return new Promise(r=>setTimeout(r,400)); }
  ...
  (function(){
    ...
    clear.addEventListener('click', ()=>{ input.value=''; setState(); input.focus(); });
    setState();
    ...
    input.addEventListener('keydown', async (e)=>{
      if(e.key === 'Enter'){
        ...
        }catch{ toast('Không thể tìm kiếm'); }
      }
    });
  })();
  ...
  (function(){
    ...
    document.getElementById('btnClearFilter').onclick = ()=>{
      ['fStatus','fClinic','fFrom','fTo'].forEach(id=>{ const el = document.getElementById(id); if(el) el.value=''; });
    };
  ...
  (function(){
    ...
    function open(id){ currentResId=id; modal.classList.add('open'); modal.setAttribute('aria-hidden','false'); }
    ...
    // open via button
    document.addEventListener('click', (e)=>{
      const btn = e.target.closest('.js-reschedule'); if(!btn) return;
      open(btn.dataset.id);
    });
    ...
    btnSave && btnSave.addEventListener('click', async ()=>{
      ...
      if(!date || !time){ toast('Vui lòng chọn ngày & giờ'); return; }
      const url = (window.APPT_API && APPT_API.reschedule) ? APPT_API.reschedule(currentResId) : '#';
      try{
        if(url !== '#') await apiFetch(url, {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({date,time})});
        else await fakeFetch();
        toast('Đã đổi lịch');
        close();
        // TODO: update UI for the item
      }catch{ toast('Không thể đổi lịch'); }
    });
  })();
  
  // ===== Cancel appointment (shared) =====
  document.addEventListener('click', async (e)=>{
    const btn = e.target.closest('.js-cancel'); if(!btn) return;
    ...
    try{
      ...
      const card = btn.closest('.item'); if(card) card.remove();
    }catch{ toast('Không thể hủy lịch'); }
  });
  ...
  (function(){
    ...
    if(tabs){
      tabs.addEventListener('click', (e)=>{
        ...
        const panel = tabs.querySelector(target); if(panel) panel.classList.add('active');
      });
    }
    ...
    if(tfForm){
      ...
      tfForm.addEventListener('click', async (e)=>{
        const btn = e.target.closest('[data-action="add_timeoff"]'); if(!btn) return;
        e.preventDefault();
        ...
        try{
          ...
          tfForm.reset(); toggleRange();
          // TODO: append new timeoff to list
        }catch{ toast('Không thể thêm ngày nghỉ'); }
      });
    }
  
    // Update working hours
    document.addEventListener('click', async (e)=>{
      const btn = e.target.closest('[data-action="update_hours"]'); if(!btn) return;
      e.preventDefault();
      ...
      }catch{ toast('Không thể lưu giờ làm'); }
    });
  })();
