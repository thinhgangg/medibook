...
(() => {
    ...
    const showConfirm = (title, body, onConfirm) => {
        ...
        newOkBtn.addEventListener('click', () => { onConfirm(); hideAllModals(); });
    };
    ...
    const renderers = {
        async overview() {
            ...
            try {
                const [stats, appointmentsStats] = await Promise.all([
                    fetchAPI('/statistics/'),
                    fetchAPI('/appointments/statistics/')
                ]);

                // SỬA LỖI: Thêm data-target để xác định trang cần chuyển hướng
                panel.innerHTML = `
                    <div class="stats-grid">
                        <div class="stat-card" data-target="users"><div class="icon"><i class="fas fa-users"></i></div><div class="stat-info"><div class="value">${stats.total_users || 0}</div><div class="label">Tổng Users</div></div></div>
                        <div class="stat-card" data-target="doctors"><div class="icon"><i class="fas fa-user-md"></i></div><div class="stat-info"><div class="value">${stats.total_doctors || 0}</div><div class="label">Tổng Bác sĩ</div></div></div>
                        <div class="stat-card" data-target="reviews"><div class="icon"><i class="fas fa-star"></i></div><div class="stat-info"><div class="value">${stats.total_reviews || 0}</div><div class="label">Tổng Đánh giá</div></div></div>
                        <div class="stat-card" data-target="appointments"><div class="icon"><i class="fas fa-calendar-check"></i></div><div class="stat-info"><div class="value">${stats.total_appointments || 0}</div><div class="label">Tổng Lịch hẹn</div></div></div>
                    </div>
                    <div class="card">
            ...
            try {
                ...
                tableBody.innerHTML = users.map(u => `
                    <tr>
                        ...
                        <td class="action-buttons"><button class="btn-icon toggle-active-btn" data-id="${u.id}" data-type="users" title="Đổi trạng thái"><i class="fas fa-power-off"></i></button></td>
                    </tr>`).join('') || `<tr><td colspan="5">Không có user nào.</td></tr>`;
            } catch (e) { panel.querySelector('tbody').innerHTML = `<tr><td colspan="5" class="error-message">${e.message}</td></tr>`; }
        },
                 ...
                 tableBody.innerHTML = doctors.map(d => `
                    <tr>
                        ...
                        <td class="action-buttons">
                            ...
                            <button class="btn-icon toggle-active-btn" data-id="${d.id}" data-type="doctors" title="Đổi trạng thái"><i class="fas fa-power-off"></i></button>
                        </td>
                    </tr>`).join('') || `<tr><td colspan="5">Không có bác sĩ nào.</td></tr>`;
            } catch (e) { panel.querySelector('tbody').innerHTML = `<tr><td colspan="5" class="error-message">${e.message}</td></tr>`; }
        },
            ...
            panel.innerHTML = `<div class="card"><div class="section-header"><h1>Quản lý Lịch hẹn</h1></div><div class="table-wrapper"><table class="table"><thead><tr><th>Bệnh nhân</th><th>Bác sĩ</th><th>Ngày hẹn</th><th>Trạng thái</th></tr></thead><tbody><tr><td colspan="4">Đang tải...</td></tr></tbody></table></div></div>`;
             try {
                ...
                tableBody.innerHTML = appointments.map(a => `
                    ...
                    </tr>`).join('') || `<tr><td colspan="4">Không có lịch hẹn nào.</td></tr>`;
            } catch(e) { panel.querySelector('tbody').innerHTML = `<tr><td colspan="4" class="error-message">${e.message}</td></tr>`; }
        },
        ...
        async specialties() {
            ...
            try {
                ...
                panel.querySelector('tbody').innerHTML = specialties.map(s => `
                    <tr>
                    ...
                    </tr>`).join('') || `<tr><td colspan="4">Chưa có chuyên khoa nào.</td></tr>`;
            } catch (e) { panel.querySelector('tbody').innerHTML = `<tr><td colspan="4" class="error-message">${e.message}</td></tr>`; }
        },
        ...
        async reviews() {
            ...
            try {
                ...
                panel.querySelector('tbody').innerHTML = reviews.map(r => `
                    <tr>
                        <td>${r.patient_name || 'N/A'}</td><td>${r.doctor_name || 'N/A'}</td><td>${'★'.repeat(r.rating)}${'☆'.repeat(5 - r.rating)}</td>
                        <td>${r.comment.substring(0, 50)}...</td><td><span class="status-badge status-${r.is_active}">${r.is_active ? 'Hiện' : 'Ẩn'}</span></td>
                        <td class="action-buttons"><button class="btn-icon toggle-active-btn" data-id="${r.id}" data-type="reviews"><i class="fas fa-eye-slash"></i></button></td>
                    </tr>`).join('') || `<tr><td colspan="6">Không có đánh giá nào.</td></tr>`;
            } catch (e) { panel.querySelector('tbody').innerHTML = `<tr><td colspan="6" class="error-message">${e.message}</td></tr>`; }
        },
    ...
    const App = {
        init() { this.setupEventListeners(); this.handleRouting(); },
        setupEventListeners() {
            window.addEventListener("popstate", () => this.handleRouting());
            document.querySelectorAll(".sidebar-nav-item").forEach(nav => nav.addEventListener("click", e => {
                e.preventDefault();
        ...
        handleRouting() { this.showPanel(location.hash.slice(1) || "overview"); },
        showPanel(name) {
        ...
        async fetchAndProcessReportData(rangeKey) {
            ...
            try {
                ...
                this.renderChart('appointmentsByDoctorChart', 'bar', Object.keys(appointmentsByDoctor), 'Số lịch hẹn', Object.values(appointmentsByDoctor));
            } catch (e) {
        ...
        getDateRange(rangeKey) {
            ...
            if (rangeKey === 'this_month') { start = new Date(now.getFullYear(), now.getMonth(), 1); end = new Date(now.getFullYear(), now.getMonth() + 1, 0); }
            if (rangeKey === 'last_month') { start = new Date(now.getFullYear(), now.getMonth() - 1, 1); end = new Date(now.getFullYear(), now.getMonth(), 0); }
            if (rangeKey === 'last_7_days') { start = new Date(); start.setDate(now.getDate() - 6); }
        ...
        renderChart(canvasId, type, labels, label, data) {
            ...
            canvas.chartInstance = new Chart(ctx, {
                type, data: { labels, datasets: [{ label, data, backgroundColor: type === 'bar' ? 'rgba(37, 99, 235, 0.6)' : 'rgba(37, 99, 235, 0.1)', borderColor: '#2563eb', fill: type === 'line', tension: 0.4 }] },
